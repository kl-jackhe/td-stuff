...
<body>
    ...
    $(document).ready(function() {
        if (LIFF_userID != "") {
            setTimeout("get_line_id_and_login('auth/edit_user')", 50);
        } else {
        	setTimeout("get_line_id_and_login('auth/edit_user')", 50);
        }
    });
    ...
    function get_line_id_and_login(url = 'auth/edit_user', redirect = 'yes') {
        if (LIFF_userID != "") {
            // alert(LIFF_userID);
            $.ajax({
                type: 'GET',
                url: '<?php echo base_url(); ?>line/check_user',
                data: 'line_id=' + LIFF_userID + '&url=auth/edit_user',
