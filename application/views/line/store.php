...
<body>
    ...
    $(document).ready(function() {
        if (LIFF_userID != "") {
            get_line_id_and_login('home')
            // setTimeout("get_line_id_and_login('home')", 50);
        } else {
        	setTimeout("get_line_id_and_login('home')", 50);
        }
    });
    ...
    function get_line_id_and_login(url = 'home', redirect = 'yes') {
        if (LIFF_userID != "") {
            // alert(LIFF_userID);
            $.ajax({
                type: 'GET',
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if(data!=0){
                                    ...
                                    window.location = '/store?county='+data[0]['county']+'&district='+data[0]['district']+'&zipcode=114&address='+data[0]['address']+'#main_area';
                                } else {
                                    // alert('0');
