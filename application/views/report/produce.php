<div class="row">
  ...
  <div class="col-md-6">
    ...
    foreach ($result as $data):
      $total_cost += $data['produce_price'];
    endforeach;
    echo '查詢: '.$this->input->get('start_date').' 到 '.$this->input->get('end_date').' 的生產費用: <br>'.'<h3>$'.number_format($total_cost).'</h3>';
    } ?>
  </div>
    ...
    <div class="table-responsive">
      ...
      <table class="table table-striped table-bordered table-hover">
        ...
        <?php foreach ($result_item as $data): ?>
          <tr>
            <td class="text-center"><?php echo $data['produce_id'] ?></td>
            <td><?php echo get_bom_product_name($data['bom_id']) ?></td>
            <td align="center"><?php echo number_format($data['produce_qty']) ?></td>
            <td align="right"><?php echo '$'.number_format($data['produce_price']) ?></td>
